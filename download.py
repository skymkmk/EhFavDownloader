...
def _get_info(gal_info: Tuple[int, str, int, str, int]) -> Union[List[Tuple[str]], None]:
    ...
    if img_count != gal_info[2]:
        ...
        img_info = pageparser.parse_gallery_img_list(url)
        if img_info is None:
...
async def _download_img(path: str, gid: int, page_num: int, ptoken: str, gal_name: str, nl: str = None,
    ...
    async with asyncio.Semaphore(config.connect_limit):
        ...
        try:
            ...
            img = await get(url, ultimate_retry_time=0)
            if img is None:
...
def download() -> None:
    ...
    for i in gal_info:
        ...
        if not os.path.exists(path):
            os.mkdir(path)
        img_info = _get_info(i)
        if img_info is not None:
            if len(img_info) != 0:
                tasks = [asyncio.create_task(_download_img(path, i[0], j[0], j[1], gal_info[3])) for j in img_info]
            ...
            if success:
                if config.save_as_cbz:
                    ...
                    year_node.text = favorited_time.year
                    month_node = ET.SubElement(root, 'Month')
                    month_node.text = favorited_time.month
                    day_node = ET.SubElement(root, "Day")
                    day_node.text = favorited_time.day
